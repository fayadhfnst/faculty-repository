<?php
    ...
    $sql3 = "SELECT * FROM surat WHERE id = $id and email_akun = '{$_SESSION['email']}'";
    $query3 = mysqli_query($con,$sql3);
    $num_row = mysqli_num_rows($query3);
    if($num_row == 0){
        echo "<script> alert('Ini bukan surat anda') </script>";
                ...
                <div class="w-full h-full flex justify-center items-center">
                    ...
                    <form method="post" enctype="multipart/form-data">
                            ...
                            if(isset($_POST['save'])){
                                ...
                                if(strlen($nama_file) != 0){
                                    ...
                                    if(!in_array(pathinfo($nama_file,PATHINFO_EXTENSION),$allowed_extensions)){
                                        ...
                                        echo "<script type='text/javascript'> document.location = 'letters-update.php?id=$id'; </script>";
                                    }else{
                                        ...
                                        $sql1 = "UPDATE surat SET judul = '$judul',id_jenis_surat = '$jenis_surat',dokumen = '$nama_file',keterangan = '$keterangan', tgl_diperlukan = '$tgl_diperlukan' WHERE id = '$id'";
                                    }
                                }else{
                                    $sql1 = "UPDATE surat SET judul = '$judul',id_jenis_surat = '$jenis_surat',keterangan = '$keterangan', tgl_diperlukan = '$tgl_diperlukan' WHERE id = '$id'";
                                }
                        ...
                        <div class="max-w-md flex flex-col justify-center items-center px-4 md:py-6">
                            <div class="w-full">
                                <input required class="w-full h-10 rounded-md shadow-md px-4 bg-gray-200 focus:outline-green-700 focus:bg-white" type="text" name="judul" id="" placeholder="Judul" value="<?=$row3['judul']?>">
                                <div class="mt-8 space-y-2">
                                    <label for="start">Tanggal diperlukan</label>
                                    <input required class="w-full h-10 rounded-md shadow-md px-4 bg-gray-200 focus:outline-green-700 focus:bg-white" type="date" id="start" name="tgl_diperlukan" value="<?=$row3['tgl_diperlukan']?>">
                                </div>
                                <div class="mt-8 space-y-2">
                                    <label for="letters">Dokumen</label>
                                    <input required class="" type="file" name="name-file" id="">
                                </div>
                                ...
                                <textarea required class="w-full h-60 mt-8 rounded-md shadow-md px-4 py-2 bg-gray-200 focus:outline-green-700 focus:bg-white" name="keterangan" id="" cols="30" rows="10" placeholder="Keterangan"><?=$row3['keterangan']?></textarea>
                            </div>
