                ...
                <div class="w-full py-2 mt-2 mb-12 flex flex-col space-y-4">
                    ...
                    <div class="w-full flex gap-8">
                        ...
                        <?php if($row2['Field'] == 'tgl_upload' || $row2['Field'] == 'tgl_pengesahan' || $row2['Field'] == 'tgl_diperlukan' || $row2['Field'] == 'tgl_terbit'){ ?>
                            <input class="rounded-md" type="date" name="<?=$row2['Field']?>" placeholder="Email" required>
                        <?php }elseif($row2['Field'] == 'file_url' || $row2['Field'] == 'file_link' || $row2['Field'] == 'file_surat'){ ?>
                            <input class="rounded-md" type="file" name="filename" required>
                        ...
                        <?php }else{ ?>
                            ...
                            <Textarea class="w-full border-2 border-gray-400 rounded-md shadow-md focus:shadow-2xl p-2" cols="30" rows="6" name="<?=$row2['Field']?>" placeholder="Masukkan Judul" required></Textarea>
                        <?php } ?>
                            <input type="text" value="<?=$nama_tabel?>" name="nama_tabel" hidden> 
                    </div>
                    <?php } ?>
                    <button class="max-w-fit h-10 px-4 rounded-md shadow-md bg-[#006633] hover:bg-green-700 active:bg-sky-700 text-white" name="add">Insert</button>
                    <?php
                        if(isset($_POST['add'])){
                            $count = count($a);
                            for($i = 0; $i < $count; $i++){
                            ...
                            if(strlen($nama_file) > 0){
                                ...
                                if($nama_tabel == 'role_akun'){
                                    $sql7 = "SELECT * FROM role_akun WHERE email = '{$email}' && tipe_akun = '{$tipe_akun}'";
                                    $query7 = mysqli_query($con,$sql7);
                                    $num_row = mysqli_num_rows($query7);

                                    if($num_row > 0){
                                        echo "<script> alert('Role sudah ada pada akun tersebut.  Insert gagal') </script>";
                                        echo "<script type='text/javascript'> document.location = 'dashboard-add.php?nama_tabel=$nama_tabel'; </script>";
                                    }
                                    else{
                                        $sql1 = "INSERT INTO $nama_tabel VALUES(";
                                        for($i = 0;$i < $count;$i++){
                                            if($a[$i] == 'file_url'){
                                                $sql1 .= "'$nama_file' ";
